package com.liweiyap.narradir.util;

public final class Constants
{
    private Constants(){}

    public static final int GAME_AVALON = 0;
    public static final int GAME_SECRETHITLER = 1;

    public static final int REQUEST_SETTINGS_HOME = 100;
    public static final int RESULT_OK_SETTINGS_HOME = 101;

    public static final int REQUEST_SETTINGS_NEW = 200;
    public static final int RESULT_OK_SETTINGS_ONESTEP = 201;
    public static final int RESULT_OK_SETTINGS_TWOSTEPS = 202;
}